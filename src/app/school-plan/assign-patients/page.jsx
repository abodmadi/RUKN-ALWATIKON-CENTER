...
const AssignPatientsToSchoolPlan = () => {
  ...
  const fetchAssignedPatients = async () => {
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/school/school-assignments`
      );
      ...
      const patientIds = assignments.map((assignment) =>
        assignment.patient && typeof assignment.patient === "object"
  ...
  const handleAssignPatient = async (patientId) => {
    ...
    try {
      ...
      alert("Patient assigned to school department successfully!");
    } catch (error) {
      console.error("Error assigning patient:", error);
      alert("Error assigning patient to school department");
    }
  };
  ...
  const handleUnassignPatient = async (patientId) => {
    try {
      await axiosInstance.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/school/unassign-from-school/${patientId}`
      );
      setAssignedPatients(assignedPatients.filter((id) => id !== patientId));
      alert("Patient unassigned from school department successfully!");
    } catch (error) {
      console.error("Error unassigning patient:", error);
      alert("Error unassigning patient from school department");
    }
  };
    ...
    <div className="card h-100 p-0 radius-12">
      <div className="card-header border-bottom bg-base py-16 px-24 d-flex align-items-center flex-wrap gap-3 justify-content-between">
        <div className="d-flex align-items-center flex-wrap gap-3">
          <h6 className="text-lg mb-0">Assign Patients to school Department</h6>
        ...
        <button
          onClick={() => router.push("/school-plan/patients")}
          className="btn btn-primary-600 radius-8 px-20 py-11"
        >
          View school Patients
        </button>
      </div>
      ...
      <div className="card-body p-24">
                      ...
                      <td className="text-center">
                        {assignedPatients.includes(patient._id) ? (
                          <button
                            ...
                            title="Unassign from school Department"
                          >
                            <Icon
                          ...
                          <button
                            ...
                            title="Assign to school Department"
                          >
                            <Icon
        ...
        <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 mt-24">
          <span className="text-gray-900">
            ...
            {Math.min(currentPage * 10, patients.length)} of {patients.length}{" "}
            patients
          </span>
...
export default AssignPatientsToSchoolPlan;
